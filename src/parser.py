...
class Parser:
    ...
    def parse_arguments():
        ...
        parser.add_argument('--debug', help='verbose', action="store_false")

        subparsers = parser.add_subparsers(title='commands to execute')

        parser_put = subparsers.add_parser('put',
                                           help='upload file to the server')
        parser_put.add_argument(
            'path1', help='path where file located on local machine')
        parser_put.add_argument(
            'path2', nargs='?', default='.',
            help='path where file will be uploaded on the server')
        ...
        parser_get.add_argument('path1',
                                help='path where file located on the server')
        parser_get.add_argument(
            'path2', nargs='?', default=DOWNLOAD_DEFAULT_PATH,
